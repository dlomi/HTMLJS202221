...
var player;
	...
	healthPotion = new GameObject({width:24, height:50, x:400, y:canvas.height/2 - 120, color:"#ff0000"});

	speedPotion = new GameObject({width:24, height:50, x:900, y:canvas.height/2 + 70, color:"#00ffff"});

	normalPotion = new GameObject({width:24, height:50, x:780, y:canvas.height/2 + 120, color:"#00ffff"});
...
function animate()
{
	...
	if(player.hitTestObject(healthPotion))
	{
		healthPotion.y = 10000;
		healthBar.width += 100;
	}
	if(player.hitTestObject(speedPotion))
	{
		player.vx = 10;
		speedPotion.y = 10000;
	}
	if(player.hitTestObject(normalPotion) && player.vx >= 10)
	{
		player.vx = 4;
		normalPotion.y = 10000;
	}
	...
	platform2.drawRect();
	platform3.drawRect();
	healthPotion.drawCircle();
	speedPotion.drawCircle();
